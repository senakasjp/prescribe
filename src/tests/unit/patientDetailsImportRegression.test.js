...
describe('PatientDetails import regression', () => {
  ...
  it('keeps PDF medication formatting rules aligned for volume and packet count output', () => {
    ...
    expect(source).toContain('pushUniquePart(`${PRESCRIPTION_DISPLAY_LABELS.volumePrefix} ${resolvedVolumeText}`)')
    expect(source).toContain('if (!supportsRightHeaderStrength(medication)) {')
    expect(source).toContain('shouldIncludeInventoryStrength && !isMeasuredLiquidMedication(medication)')
    expect(source).toContain('if (!supportsRightHeaderStrength(medication))')
    expect(source).toContain('const fromInventory = findInventoryStrengthText(medication)')
    expect(source).toContain('pushUniquePart(qtsMetaLine)')
