...
describe('PatientDetails import regression', () => {
  ...
  it('keeps PDF medication formatting rules aligned for volume and packet count output', () => {
    const __filename = fileURLToPath(import.meta.url)
    const __dirname = path.dirname(__filename)
    const patientDetailsPath = path.resolve(__dirname, '../../components/PatientDetails.svelte')
    const source = fs.readFileSync(patientDetailsPath, 'utf8')

    expect(source).toContain("dosageForm === 'liquid (measured)'")
    expect(source).toContain("import {")
    expect(source).toContain('PRESCRIPTION_DISPLAY_LABELS')
    expect(source).toContain('if (!isLiquidMedication(medication) && !requiresQtsPricing(medication))')
    expect(source).toContain('if (usesVolumeLabel)')
    expect(source).toContain('parts.push(`${PRESCRIPTION_DISPLAY_LABELS.volumePrefix} ${resolvedVolumeText}`)')
    expect(source).toContain('/\\b(?:ml|l)\\b/i.test(strengthText)')
    expect(source).toContain('&& !isMeasuredLiquidMedication(medication)')
    expect(source).toContain('&& !isBottledLiquidMedication(medication)')
    expect(source).toContain('shouldIncludeInventoryStrength && !isMeasuredLiquidMedication(medication)')
    expect(source).toContain('parts.push(qtsMetaLine)')
    expect(source).toContain('return `${formLabel} | ${PRESCRIPTION_DISPLAY_LABELS.quantityPrefix} ${String(parsedQts).padStart(2, \'0\')}`')
    expect(source).not.toContain('parts.push(`${usesVolumeLabel ? \'Vol\' : \'Strength\'}: ${inventoryStrengthText}`)')
    expect(source).not.toContain('parts.push(`Pack: ${packMlText}`)')
  })
})
